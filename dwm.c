...
void monocle(Monitor *m) {
  ...
  for (c = nexttiled(m->clients); c; c = nexttiled(c->next))
    resize(c, m->wx + vp, m->wy + vp, m->ww - 2 * c->bw - 2 * vp, m->wh - 2 * c->bw -  2 * vp, 0);
}
...
void tile(Monitor *m) {
  ...
  for (i = 0, my = ty = m->gappx, c = nexttiled(m->clients); c;
    ...
    if (i < m->nmaster) {
      r = MIN(n, m->nmaster) - i;
      h = (m->wh - my - m->gappx * (r - 1)) /
          r; // (MIN(n, m->nmaster) - i) - m->gappx;
      resize(c, m->wx + m->gappx, m->wy + my, mw - (2 * c->bw) - m->gappx,
             h - (2 * c->bw) - m->gappx, 0);
      if (!(nexttiled(c->next)))
        resize(c, m->wx + m->gappx, m->wy + my, mw - (2 * c->bw) - m->gappx * 2,
               h - (2 * c->bw) - m->gappx, 0);
      my += HEIGHT(c) + m->gappx;
    } else {
      smh = m->mh * m->smfact;
      ...
      if (h < minwsz) {
        c->isfloating = True;
        XRaiseWindow(dpy, c->win);
        resize(c, m->mx + m->gappx + (m->mw / 2 - WIDTH(c) / 2),
               m->my + (m->mh / 2 - HEIGHT(c) / 2) - m->gappx * 2,
               m->ww - mw - (2 * c->bw) - gappx, h - (2 * c->bw) - m->gappx * 2,
               0);
        ty -= HEIGHT(c) + m->gappx;
      } else
        resize(c, m->wx + m->gappx + mw, m->wy + ty,
               m->ww - mw - (2 * c->bw) - m->gappx * 2,
               h - (2 * c->bw) - m->gappx, 0);
      if (!(nexttiled(c->next)))
...
void updatebars(void) {
  ...
  for (m = mons; m; m = m->next) {
    ...
    m->barwin = XCreateWindow(
        dpy, root, m->wx + sp, m->by + vp, m->ww - 2 * sp, bh, 0, DefaultDepth(dpy, screen),
